...
public class CartaoController {
    ...
    public ResponseEntity<Cartao> buscarCartaoID(@PathVariable Integer numeroCartao){
        Long num = new Long(numeroCartao);

        Optional<Cartao> cartaoOptional = cartaoService.buscarPorId(num);
    ...
    public ResponseEntity<Cartao> atualizarCartao(@PathVariable Long id ,@RequestBody @Valid Cartao cartao) {
        ...
        try {
            ...
            return ResponseEntity.status(201).body(cartaoAtualizado);
        } catch (Exception ex) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ex.getMessage());
        }
    }
